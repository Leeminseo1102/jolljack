...
function saveResult(res) {
    sessionStorage.setItem('cropAnalysisResult', JSON.stringify(res));
}
...
function handleLoadingResponse(res) {
    ...
    if (res.result === 'SESSION_INVALID' || res.result === 'REGION_NOT_FOUND') {
        moveWithModal('지역 정보를 확인해주세요', res.msg, CTX + 'crop/region');
        return;
    }

    moveWithModal('분석을 진행할 수 없습니다', res.msg || '작물 분석 처리 중 오류가 발생했습니다.', CTX + 'crop/region');
}
...
function runLoadingProc() {
    ...
    ajaxPost(CTX + 'crop/loadingProc', {}, function (res) {
